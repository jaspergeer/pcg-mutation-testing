...
pub struct ShareMutablyLent;

impl PeepholeMutator for ShareMutablyLent {
    fn generate_mutants<'mir, 'tcx>(
        ...
        let immutably_lent_in_curr = {
            ...
            let mut owned_lent = {
                ...
                filter_owned_places_by_capability(&owned_capabilities, |ck| ck == CapabilityKind::Lent)
            };
            let mut borrowed_lent = {
                let borrow_capabilities = borrows_state.capabilities.as_ref();
                filter_borrowed_places_by_capability(&borrow_capabilities, |ck| ck == CapabilityKind::Lent)
            };
